...
/**
 * تنبيه ولي الأمر عند فتح تطبيق مستخدم / مراسلة (مطابق لـ AppUsageAlertHelper في Kotlin).
 */
object AppUsageAlertHelper {
    ...
    fun onForegroundChanged(context: Context, packageName: String?) {
        ...
        val childCode = ChildContextStore.getChildCode(app)
        ...
        val message = when {
            MonitoredAppRegistry.isMessagingApp(pkg) ->
                "مراقبة محادثات — الطفل فتح $label ($category)"
            category == "YouTube" ->
                "مراقبة تطبيق — الطفل فتح YouTube"
            else ->
                "مراقبة تطبيق — الطفل يستخدم: $label ($category)"
        }

        scope.launch {
            if (NetworkClientLite.addAlert(childCode, message)) {
