...
DEFAULT_ANDROID = PROJECT_ROOT / "MYRana"
...
def load_config(path: Path | None = None) -> DevicesConfig:
    ...
    raw_android = data.get("androidProject")
    if raw_android:
        android_project = Path(str(raw_android)).expanduser()
        if not android_project.is_absolute():
            android_project = PROJECT_ROOT / android_project
    else:
        android_project = DEFAULT_ANDROID
